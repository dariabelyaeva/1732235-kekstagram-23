...
const COMMENTS_IDS = new Array(USERS).fill('').map((value, id) => id + 1).sort(() => Math.random() - 0.5);
const PUBLICATIONS_IDS = new Array(PHOTO_COUNT).fill('').map((value, id) => id + 1);
const PHOTOS_URLS = new Array(PHOTO_COUNT).fill('').map((value, id) => `photos/${id + 1}.jpg`);
...
const COMMENTS = new Array(PHOTO_COUNT).fill().map(() => createComments());
...
const createPublication = () => ({
  ...
  comments: getUniqueArrayElement(COMMENTS),
});
